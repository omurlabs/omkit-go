...
// WithRequestIDFromContext auto-sets X-Request-ID on every outbound request
// from requestid.FromContext(ctx). No-op when ctx has no id or the caller
// already set the header on the request. Intended for every service-to-service
// hop so a single correlation id traces a request across spine → cortex →
// synapse → marrow without each callsite re-extracting it.
func WithRequestIDFromContext() Option {
	return func(c *Client) { c.requestIDFromContext = true }
}
